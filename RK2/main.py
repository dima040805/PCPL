...
def create_many_to_many(procedures, databases, procedures_in_dbs):
    many_to_many_temp = [
...
def task1(one_to_many):
    return [(x[0], x[2]) for x in one_to_many if x[0].startswith('g')]
...
def task3(many_to_many):
    ...
    for proc_name, proc_priority, db_name in sorted(many_to_many, key=itemgetter(0)):
        res.append(f'Процедура {proc_name} с приоритетом {proc_priority} находится в базе данных {db_name}')
    return res
...
def main():
    ...
    many_to_many = create_many_to_many(procedures, databases, procedures_in_dbs)
